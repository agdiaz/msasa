...
from generators import AlignmentGenerator
...
class NpSimulatedAnnealing():
	...
	def __init__(self, input_file, sequences_comparer, inner_loop = False):
		...
		self.neighbor_generator = AlignmentGenerator()


	def execute(self, n_iterations: int, initial_temp: float, _is_debugging=False):
		# if is_debugging:
		...
		results = Results()
		results.set_initial(self.initial, self.score_function.np_calculate_score(self.initial))

		# generate an initial point and evaluate the initial point
		curr = self.neighbor_generator.realign_sequences(self.initial)
		curr_eval = self.score_function.np_calculate_score(curr)


		for iteration_random, changes, curr_temp in iterations:
			# evaluate candidate point

			iteration_candidate, iteration_candidate_score = curr, curr_eval
			if self.inner_loop:
				for _inner_loop_index in range(10):
					candidate      = self.neighbor_generator.realign_sequences(curr, changes)
			...
			if diff == 0: # self.optimization.is_better_than_best(diff):
				...
				curr, curr_eval = candidate, candidate_eval #, NpSimulatedAnnealing.CRITERIA_BETTER
				no_changes_count = 0
			elif metro(diff, curr_temp, iteration_random):
				# store the new current point even if it is not better than the current one
				curr, curr_eval = candidate, candidate_eval #, NpSimulatedAnnealing.CRITERIA_METROPOLIS
				no_changes_count = 0
			else:
				no_changes_count += 1
