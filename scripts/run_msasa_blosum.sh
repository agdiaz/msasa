...
do
	OUTPUT_MSA_FILENAME=$OUTPUT_FOLDER/$filename/$i.msasa_blosum.fa
	PLOT_BEST_FILENAME=$OUTPUT_FOLDER/$filename/$i.msasa_blosum_best.png
	PLOT_TEMP_FILENAME=$OUTPUT_FOLDER/$filename/$i.msasa_blosum_temp.png
	echo "MSASA Execution # $i started"
	python3 $MSASA --input $INPUT_FASTA --output $OUTPUT_MSA_FILENAME --comparer blosum --n-iterations 500 --output-best-plot $PLOT_BEST_FILENAME --output-temp-plot $PLOT_TEMP_FILENAME --optimization min >> $LOG_FILENAME
done
